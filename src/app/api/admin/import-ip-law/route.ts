...
import { finalIPArticles, finalIPQAs } from '@/data/ip-law-final-content'

// IP Law Category ID
const IP_LAW_CATEGORY_ID = '64f9abe4-f1c2-4eb6-9d11-6f107ab9def1'

// Use final IP Law content from data file
const ipLawArticles = finalIPArticles;
const ipLawQAs = finalIPQAs;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting IP Law content import...')
    ...
    // Import Articles
    console.log(`Importing ${ipLawArticles.length} IP Law articles...`)
    
    for (const articleData of ipLawArticles) {
      try {
    ...
    // Import Q&As
    console.log(`Importing ${ipLawQAs.length} IP Law Q&As...`)
    
    for (const qaData of ipLawQAs) {
      try {
        const qa = {
          category_id: IP_LAW_CATEGORY_ID,
          user_id: null,
    ...
    // Summary
    console.log('IP Law content import completed:')
    ...
    console.error('IP Law import error:', error)
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to import IP Law content',
...
// GET endpoint to check current IP Law content status
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      import_data: {
        articles_to_import: ipLawArticles.length,
        qas_to_import: ipLawQAs.length
      }
    })

  } catch (error) {
    console.error('Error checking IP Law content:', error)
    return NextResponse.json(
      { 
        success: false, 
